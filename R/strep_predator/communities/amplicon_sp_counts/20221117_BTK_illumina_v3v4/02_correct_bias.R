...
# Read data ---------------------------------------------------------------
counts    <- readr::read_rds(here::here("data", "amplicon", "species_counts.rds"))
...
observed_mat <- left_join(counts, expdesign) %>% 
  filter(str_detect(sample, "positive_control")) %>%
  left_join(., pos_ctrl) %>%
...
samples <- c("T4BT2_t1", "T4BTS2_t1", "T4BS2_t1", "ANCMIX1_t0", "BT1_t0")
...
counts_final_fmt <- counts_final %>%
  ...
  filter(!str_detect(sample, "positive_control")) %>%
  dplyr::select(sample, strainID, genus, species, count, count_correct)
  
write_rds(counts_final_fmt, here::here("data", "amplicon", "corrected_species_counts.rds"))